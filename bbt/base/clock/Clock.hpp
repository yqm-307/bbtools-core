...
// 时间值
BBT_TIME_CONVERT_TYPE scMinOfHour = 60;
BBT_TIME_CONVERT_TYPE scHourOfDay = 24;
BBT_TIME_CONVERT_TYPE scDayOfWeek = 7;
BBT_TIME_CONVERT_TYPE scSecOfMin = 60;
BBT_TIME_CONVERT_TYPE scSecOfHour = scSecOfMin * scMinOfHour; 
BBT_TIME_CONVERT_TYPE scSecOfDay = scSecOfMin * scMinOfHour * scHourOfDay;
BBT_TIME_CONVERT_TYPE scSecOfWeek = scSecOfDay * scDayOfWeek;
 ...
 */
inline time_t DayOfMonth(Timestamp<ns>&& ts = now<ns>())
{
 ...
 */
inline time_t MonthOfYear(Timestamp<ns>&& ts = now<ns>())
{
 ...
 * @return time_t (0-23)
 */
inline time_t HourOfDay(Timestamp<ns>&& ts=now<ns>())
{
    time_t s = utcms(std::move(ts)) / 1000;    //ms
    return (s % (86400)) / (3600);
}
 ...
 * @return time_t 当前年份
 */
inline time_t YearOf(Timestamp<ns>&& ts=now<ns>())
{
    time_t tt = system_clock::to_time_t(ts);
    tm utc_tm = *gmtime(&tt);
 ...
 * @return time_t 
 */
inline time_t MinuteOfHour(Timestamp<ns>&& ts = now<ns>())
{
    time_t tt = system_clock::to_time_t(ts);
    tm utc_tm = *gmtime(&tt);
 ...
 * @return time_t 
 */
inline time_t SecondOfMinute(Timestamp<ns>&& ts = now<ns>())
{
    time_t tt = system_clock::to_time_t(ts);
    tm utc_tm = *gmtime(&tt);
 ...
 * @return time_t 
 */
inline time_t MsOfSecond(Timestamp<ns>&& ts = now<ns>())
{
...
{
    return std::to_string(YearOf())+'/'+std::to_string(MonthOfYear()+1)+'/'+std::to_string(DayOfMonth())+'/'
            +std::to_string(HourOfDay())+'/'+std::to_string(MinuteOfHour())+'/'+std::to_string(SecondOfMinute())+'/'+std::to_string(MsOfSecond());
}
...
static inline uint32_t ZoneTime()
{
    ...
    int offset_sec = ((( utc_now - gmt_now )) + ( isdst ? scSecOfHour : 0 ));
